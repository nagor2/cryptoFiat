...
before('should setup the contracts instance', async () => {
    ...
    dao = await INTDAO.deployed([0x0, cdp.address, auction.address, 0x0, 0x0, rule.address, flatCoin.address, 0x0]);
    ...
    it("should transfer interest to CDP and decease recorded fee if not restricted", async () => {
        ...
        assert.equal(parseFloat(await cdp.totalCurrentFee(posId)/10**18).toFixed(4), 99.0000, "wrong total fee");
        await flatCoin.approve (cdp.address, web3.utils.toWei('99.01'), {from: position.owner});
        ...
        assert.equal(await flatCoin.balanceOf(cdp.address), web3.utils.toWei('0'), "wrong cdp balance");

        await cdp.transferInterest(posId, {from: position.owner});

        assert.equal(parseFloat(await flatCoin.balanceOf(cdp.address)/10**18).toFixed(4), 99.0000, "wrong cdp balance");
        assert.equal(parseFloat(await flatCoin.balanceOf(position.owner)/10**18).toFixed(4), 1.0000, "wrong owner balance");
...
it("should generate additional fee", async () => {
    ...
    await flatCoin.transfer(recipient, web3.utils.toWei('950', 'ether'), {from:owner});

    actualBalance = await flatCoin.balanceOf(position.owner);

    let recipientBalance = await flatCoin.balanceOf(recipient);
    ...
    await flatCoin.transfer(owner, web3.utils.toWei('50', 'ether'), {from:recipient});
    await flatCoin.approve(cdp.address, web3.utils.toWei('100', 'ether'), {from:owner})
    await cdp.transferInterest(posId);
    const feeAfter = await cdp.totalCurrentFee(posId);

    let cdpBalance = await flatCoin.balanceOf(cdp.address);
    ...
    it("should allow and transfer surplus to auction and create RuleBuyOut", async () => {
        ...
        let cdpBalance = parseFloat(await flatCoin.balanceOf(cdp.address)/10**18).toFixed();

        let surplus = (cdpBalance - parseFloat(await flatCoin.totalSupply()/10**18)*(parseFloat(await dao.params('stabilizationFundPercent')))/100);
        ...
        truffleAssert.eventEmitted(buyOutInitTx, 'newAuction', async (ev) => {
            ...
            assert.equal(ev.lotAddress, flatCoin.address, "Should be correct address");
