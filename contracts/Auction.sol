...
contract Auction is ReentrancyGuard{
    ...
    /// @param auctionType Type of the auction 1 - Rule buyout, 2 - flatcoins buyout, 3 - collateral liquidation
    ...
    /// This is non-zero only if topping up the stabilization fund with a certain amount of flatcoins.
    ...
    /// @notice Initiates a flatcoin buyout for the stabilization fund if it is low. The stabilization fund should always be a certain percentage of the total flatcoin supply (as defined by the stabilizationFundPercent parameter in the DAO contract). The maximum flatcoins per auction is regulated by the maxCoinsForStabilization parameter in the DAO contract.
    ...
    function improveBid(uint32 bidID, uint256 newBidAmount) nonReentrant external{
        ...
        if (a.auctionType == 2)
            require(newBidAmount>0 && bestBid.amount *(100-dao.params("minAuctionPriceMove"))/100>=newBidAmount, "not low enough");
        else
            require(newBidAmount>0 && bestBid.amount *(100+dao.params("minAuctionPriceMove"))/100<=newBidAmount, "not high enough");

        IERC20 paymentToken = IERC20(address(a.paymentToken));

        if (a.auctionType == 1 || a.auctionType == 3){
            uint256 difference = newBidAmount-b.amount;
            require(paymentToken.transferFrom(msg.sender, address_this, difference), "You should first approve payment to auction contract address");
        }

        b.amount = newBidAmount;
        a.lastTimeUpdated = block.timestamp;
        a.bestBidID = bidID;
    ...
    function claimToFinalizeAuction(uint32 auctionID) nonReentrant external returns (bool success){
        ...
        require(block.timestamp-a.lastTimeUpdated>=dao.params("auctionTurnDuration"), "too early");
        require(a.bestBidID !=0 && a.initTime!=a.lastTimeUpdated, "no bid");
